...
const validarCampos = (estudiante) => {
  ...
  else if (!reglas.soloLetras.test(estudiante.nacionalidad))
...
function EstudianteEditForm() {
  const { id } = useParams();
  const navigate = useNavigate();

  const [estudiante, setEstudiante] = useState(null);
  const [encargadosOriginales, setEncargadosOriginales] = useState([]); // snapshot al cargar
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      setCargando(true);
      try {
        // Cargar estudiante y sus encargados en paralelo
        const [data, relaciones] = await Promise.all([
          estudianteService.getEstudianteById(id),
          encargadoService.getEncargadosByEstudiante(id),
        ]);
        ...
        // Separar encargado legal y adicionales desde las relaciones
        const legal = relaciones.find((r) => r.esEncargadoLegal);
        const adicionales = relaciones.filter((r) => !r.esEncargadoLegal);

        const encargadoLegal = legal
          ? { ...legal.encargado, parentesco: legal.parentesco }
          : null;

        const encargados = adicionales.map((r) => ({
          ...r.encargado,
        ...
        // Guardar snapshot original para comparar cambios al guardar
        setEncargadosOriginales(relaciones);
  ...
  // Handlers encargados adicionales 
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      await estudianteService.putEstudiante(id, {
        id: parseInt(id),
        ...
        gradoEscolarId: estudiante.gradoEscolarId || null,
      });

      // Calcular diferencias en encargados
      const estudianteId = parseInt(id);

      // IDs originales
      const idsOriginales = encargadosOriginales.map((r) => r.encargadoId);

      // IDs actuales (legal + adicionales)
      const idsActuales = [
        estudiante.encargadoLegal.id,
        ...estudiante.encargados.map((e) => e.id),
      ];

      // Borrar los que ya no están
      const aEliminar = encargadosOriginales.filter(
        (r) => !idsActuales.includes(r.encargadoId)
      );
      for (const r of aEliminar) {
        await estudianteService.deleteEncargadoEstudiante(estudianteId, r.encargadoId);
      }

      // Insertar los nuevos
      if (!idsOriginales.includes(estudiante.encargadoLegal.id)) {
      ...
      for (const enc of estudiante.encargados) {
        if (!idsOriginales.includes(enc.id)) {
          await estudianteService.postEncargadoEstudiante({
            estudianteId,
            encargadoId: enc.id,
            ...
            <Row>
              <Col md={12}>
                <Form.Group className="mb-2">
                  ...
                  />
                  <Form.Control.Feedback type="invalid">{errores.padecimientos}</Form.Control.Feedback>
                </Form.Group>
              </Col>
            </Row>
          </Card.Body>
...
export default EstudianteEditForm;